...
async function createUser({ email, password, role }) {
  ...
  const result = await query('INSERT INTO users (name, email, password, role) VALUES ($1, $2, $3, $4) RETURNING id, name, email, role, created_at', [normalizedEmail.split('@')[0], normalizedEmail, hash, role]);
  return mapUser(result.rows[0]);
}
